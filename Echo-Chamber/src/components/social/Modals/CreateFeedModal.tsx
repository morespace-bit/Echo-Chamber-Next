...
interface ICreateFeedModalProps {
  setImageUpload: Dispatch<SetStateAction<boolean>>;
}

export default function CreateFeedModal({
  setImageUpload,
}: ICreateFeedModalProps) {
  ...
  async function uploadPost() {
    ...
    const res = await api.post("/api/createPost", post);
  ...
  const { mutate, isError, isPending } = useMutation({
    ...
    onSuccess: () => {
      client.invalidateQueries({ queryKey: ["getPost"] });
  ...
  function handleClick() {
    if (post.content.length == 0 || post.imageUrl.length == 0) {
      alert("Please upload a image and put valid caption");
    }
    mutate();
  }
        ...
        <div className="w-full flex items-center justify-center mt-4">
          {loding ? (
            ...
            <button
              ...
              onClick={handleClick}
            >
              {isPending ? "Data is being uploaded" : "Post"}
            </button>
          )}
        </div>
      </div>
